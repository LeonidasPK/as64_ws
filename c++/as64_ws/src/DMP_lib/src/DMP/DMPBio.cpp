...
DMP(N_kernels, a_z, b_z, cs_ptr, std_K, USE_GOAL_FILT, a_g)
{}


double DMPBio::shape_attractor(const arma::vec X, double g, double y0)
{
	double x,u;
  ...
  double s_attr = f * K * u - K * (g - y0) * u;
    
  return s_attr;
}

void DMPBio::calculate_Fd(const arma::rowvec &yd_data, const arma::rowvec &dyd_data, const arma::rowvec &ddyd_data, arma::mat &u, arma::rowvec &g, arma::rowvec &y0, arma::rowvec &Fd)
{
  ...
  Fd = (ddzd_data + g_attr_data + this->a_z * this->b_z * (g - y0) % u);
}
